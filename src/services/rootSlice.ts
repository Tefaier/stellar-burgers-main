...
import { forgotPasswordApi, getUserApi, loginUserApi, logoutApi, registerUserApi, resetPasswordApi, TRegisterData, updateUserApi } from '../utils/burger-api';
...
export const resetPasswordThunk = createAsyncThunk(
    ...
    async (password: string) => {
        ...
        return resetPasswordApi({password, token: getCookie("accessToken")!}).then(response => {
            return response;
        });
    }
)
...
export const userSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(registerUserThunk.fulfilled, (state: UserState, {payload}) => {
            state.isLoading = false;
            state.user = payload;
        });
    ...
    selectors: {
        selectIsLoading: (state: UserState) => {
            return state.isLoading;
        },
        selectUser: (state: UserState) => {
            return state.user;
        }
    }
});
